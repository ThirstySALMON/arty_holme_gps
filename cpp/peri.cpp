...
// Pi 3 Model B (BCM2837) peripheral base. Holme's original was 0x20000000
// (BCM2835, original Pi/Zero). Pi 2 (BCM2836) and Pi 3 (BCM2837) share the
// SAME register layout as BCM2835 — only this base address differs.
...
int peri_init() {
    ...
    // ---- FPGA programming over Pi GPIO: STUBBED OUT ----
    // Holme loaded the FPGA bitstream through the Pi (FPGA_PROG/INIT_B
    // handshake). We program the Arty A7 over its USB-JTAG from Vivado, so
    // these wait loops would block forever (INIT_B is not wired). Skip them.
    printf("Skipping Pi-driven FPGA programming (Arty is JTAG-programmed).\n");
